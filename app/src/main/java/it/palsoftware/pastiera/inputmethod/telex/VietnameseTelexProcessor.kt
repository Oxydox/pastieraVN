    ...
    private fun applyShapeKey(syllable: String, keyChar: Char): String? {
        ...
        for (idx in chars.indices.reversed()) {
            ...
            val replacement = when (key) {
                // For self-doubling shape keys (aa->â, ee->ê, oo->ô), only reach back through
                // trailing text that's empty or exactly "u" -- the one confirmed real Vietnamese
                // pattern needing this (e.g. "dau"+a->"dau"-with-circumflex, trailing "u").
                // Anything else -- a coda consonant (e.g. "mam"+a, an earlier syllable already
                // closed) or another vowel like "y" (e.g. "eye"+e, not a real Vietnamese vowel
                // cluster) -- means this isn't a legitimate reach-through, so the new keystroke
                // starts fresh instead of touching the earlier vowel.
                'a' -> when {
                    trailing.isNotEmpty() && trailing.lowercase() != "u" -> null
